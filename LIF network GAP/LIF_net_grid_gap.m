...
%% CONNECTIVITY
gEE=1;                  % mA/cm^2 (gap-junction synapses)
S_EE=M_tor(sqrt(N));    % adjacency matrix

% coupled element indexes
l=zeros(N,4);
for i=1:1:N 
...
subplot(2,2,4);
imagesc((reshape((IN +I_EE'),sqrt(N),sqrt(N)))',[0 gEE*4]); % Voltage distribution
set(gca,'Ydir','normal');
set(gca,'FontSize',20);             % set the axis with big font
colorbar;
